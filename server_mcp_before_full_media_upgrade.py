...
# This keeps every existing REST endpoint working and additionally exposes
# ChatGPT-compatible MCP tools at POST /mcp.
# No extra Python package is required; the bridge uses JSON-RPC 2.0 directly.

from fastapi.responses import JSONResponse, Response

MCP_SERVER_NAME = "gemini-voice-studio"
MCP_SERVER_VERSION = "1.0.0"
MCP_DEFAULT_PROTOCOL = "2025-06-18"


def _mcp_public_base(request: Request) -> str:
    """Return a URL that ChatGPT can use to download generated files."""
    base = str(request.base_url).rstrip("/")
    if "127.0.0.1" in base or "localhost" in base:
        base = PUBLIC_BASE_URL
    return base
...
def _mcp_tool_result(payload, *, is_error: bool = False):
    """Return both readable text and structured JSON to ChatGPT."""
    return {
        "content": [
            {
                "type": "text",
    ...
    return [
        {
            ...
            "description": (
                "Generate a downloadable Gemini voiceover audio file. "
                "Use Charon by default for YouTube voiceovers. Returns MP3/WAV download URLs."
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "Exact text to speak. Do not include voice-direction labels unless they should be spoken.",
                    },
                    "voice": {
                        "type": "string",
                        "default": "Charon",
                        "description": "Gemini voice name. Defaults to Charon.",
                    },
                    "tone": {
                        "type": "string",
                        "default": "",
                        "description": "Tone preset ID or a natural-language delivery instruction.",
                    },
                    "language": {
                        "type": "string",
                        "default": "Hindi",
                        "description": "Spoken language, e.g. Hindi or English.",
                    },
                    "filename": {
                        "type": ["string", "null"],
                        "description": "Optional output filename without requiring an extension.",
                    },
                },
                "required": ["text"],
        ...
        },
        {
            ...
            "description": (
                "Generate scene-by-scene voiceover audio and optionally merge all scenes into one master MP3/WAV. "
                "Defaults to the Charon voice."
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
                    "scenes": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "voice": {
                                    "type": ["string", "null"],
                                    "description": "Optional per-scene voice override; otherwise default_voice is used.",
                                },
                            },
                            "required": ["text"],
                            "additionalProperties": False,
                    ...
                    "master_name": {
                        "type": "string",
                        "default": "master_voiceover",
                    },
                },
                "required": ["scenes"],
                "additionalProperties": False,
            },
            "title": "Generate voiceover timeline",
            "outputSchema": {
                "type": "object",
                "properties": {"success": {"type": "boolean"}},
                "required": ["success"],
        ...
        {
            "name": "list_voices",
            "description": "List voice profiles supported by this Gemini Voice Studio server.",
            "inputSchema": {
                "type": "object",
                "properties": {},
                "additionalProperties": False,
            ...
            "outputSchema": {
                ...
                "required": ["success", "default_voice", "voices"],
                "additionalProperties": True,
            },
            "annotations": {
                "readOnlyHint": True,
...
def _mcp_generate_single(args: dict, request: Request):
    text = str(args.get("text", ""))
    if not text.strip():
        raise ValueError("Text cannot be empty.")

    voice = str(args.get("voice") or "Charon")
    ...
    base = _mcp_public_base(request)
    fn_wav = result["filename_wav"]
    fn_mp3 = result["filename_mp3"]
    result["download_url_wav"] = f"{base}/output/{fn_wav}"
    result["download_url_mp3"] = f"{base}/output/{fn_mp3}"
    result["direct_download_link"] = f"{base}/output/{fn_mp3}"
    return {"success": True, "data": result}


def _mcp_generate_timeline(args: dict, request: Request):
    raw_scenes = args.get("scenes") or []
    if not raw_scenes:
    ...
    default_voice = str(args.get("default_voice") or "Charon")
    merge_master = bool(args.get("merge_master", True))
    master_name = str(args.get("master_name") or "master_voiceover")
    ...
    for sc in result.get("scenes", []):
        if sc.get("filename_wav"):
            sc["download_url_wav"] = f"{base}/output/{sc['filename_wav']}"
        if sc.get("filename_mp3"):
            sc["download_url_mp3"] = f"{base}/output/{sc['filename_mp3']}"

    if result.get("master_audio"):
        result["master_audio"]["download_master_wav_url"] = f"{base}/output/{master_name}.wav"
        result["master_audio"]["download_master_mp3_url"] = f"{base}/output/{master_name}.mp3"
        result["master_audio"]["direct_download_link"] = f"{base}/output/{master_name}.mp3"
...
async def mcp_endpoint(request: Request):
    """Stateless Streamable-HTTP-style MCP endpoint for ChatGPT tool calls."""
    try:
        body = await request.json()
    except Exception:
        return JSONResponse(
            _mcp_error(None, -32700, "Parse error: request body must be valid JSON."),
            status_code=400,
        )

    if not isinstance(body, dict):
        return JSONResponse(
            _mcp_error(None, -32600, "Invalid Request: expected a JSON-RPC object."),
            status_code=400,
        )
    ...
    if body.get("jsonrpc") != "2.0" or not isinstance(method, str):
        return JSONResponse(
            _mcp_error(request_id, -32600, "Invalid Request."),
            status_code=400,
        )
    ...
    if method == "initialize":
        ...
        protocol_version = requested_version or MCP_DEFAULT_PROTOCOL
        return JSONResponse(
            _mcp_ok(
                request_id,
                {
                    ...
                    "instructions": (
                        "Use generate_single_audio for one voiceover and generate_timeline for scene-based voiceovers. "
                        "Use Charon unless the user explicitly requests another voice. Return the MP3 download URL to the user."
                    ),
                },
            )
        )
    ...
    if method == "tools/call":
        ...
        try:
            if name == "generate_single_audio":
                payload = _mcp_generate_single(arguments, request)
            elif name == "generate_timeline":
                payload = _mcp_generate_timeline(arguments, request)
            elif name == "list_voices":
                payload = {
                    "success": True,
                    "default_voice": "Charon",
                    "voices": [
                        {"name": voice_name, **voice_info}
                        for voice_name, voice_info in VOICE_PROFILES.items()
                    ],
                }
            else:
                return JSONResponse(
                    _mcp_error(request_id, -32602, f"Unknown tool: {name}"),
                    status_code=400,
                )
            ...
            # Tool execution failures are returned as MCP tool errors so ChatGPT
            # can read and explain the actual generation error.
            return JSONResponse(_mcp_ok(request_id, _mcp_tool_result(payload, is_error=True)))

    return JSONResponse(
        _mcp_error(request_id, -32601, f"Method not found: {method}"),
        status_code=404,
    )
